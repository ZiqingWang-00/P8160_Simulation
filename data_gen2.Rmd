...
check_pct_identified <- function(data_obj, fit_obj){
  ...
  # check how many strong/wbc/wai indicators were correctly identified
  ...
  pct_null_identified <- length(intersect(null_ind_name, fit_obj$var_selected))/length(null_ind_name) * 100
...
### Forward selection
Count the probability of successfully identifying strong, WAI, WBC predictors using forward selection:
...
varying_corr_out_forward
```

```{r}
# mean percentages of strong/wai/wbc indicators correctly identified
mean(pct_strong_vec)
mean(pct_wai_vec)
mean(pct_wbc_vec)
mean(type1_error_vec)
```
